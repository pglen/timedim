...
/* =====[ NotePad.c ]=========================================================
    ...
    Access library for debug pad. Converted to CPP to allow access
    from MFC library. Converted to Class.
    ...
    Revisions:
	  ...
	  0.00  06/12/06  Peter Glen    Class conversion
...
//////////////////////////////////////////////////////////////////////////
// The one and only notepad instance
...
int     NotePad::PrintToNotepad(const char *Format, ...)
...
int     NotePad::DumpToNotepad(void *dptr, int len)

{
    ...
    while(TRUE)
        ...
        if(slen >=  sizeof(szOutString)-46)        /* end of local buffer */
            break;

        slen += sprintf(szOutString + slen, "  -  ");

        for(aa = 0; aa < 8; aa++)
            slen += sprintf(szOutString + slen, "%02x ", *(ptr+aa) & 0xff);
        ...
        for(aa = 0; aa < 8; aa++)
            ...
            slen += sprintf(szOutString + slen, "%c", chh);
            }
        slen += sprintf(szOutString + slen, "  -  ");
        slen += sprintf(szOutString + slen, "\r\n");
...
static NotePad::int	 ToNotepadW(LPWSTR *str)
...
int     NotePad::ToNotepad(char  *str)
...
BOOL    NotePad::EnumChildProc(HWND  hwnd, LPARAM  lParam)

{
	NotePad *np = (NotePad*)lParam;
...
int     NotePad::is_nonclient(int msg)
...
int     NotePad::PrintToNotepadW(const char *Format, ...)
...
char    *NotePad::wide2str(LPWSTR pszW)
...
DWORD   NotePad::UnicodeToAnsi(LPWSTR pszW, LPSTR* ppszA)
{
...
DWORD   NotePad::AnsiToUnicode(LPCSTR pszA, LPWSTR* ppszW)
...
int     NotePad::AfxPrintf(const char *Format, ...)
...
int		NotePad::AfxDump(void *mem, int len)
